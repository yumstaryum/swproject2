...
void loop() {
  ...
  int servoAngle = 90; // Default to neutral position

  if (dist_raw < _TARGET_LOW) {
     servoAngle = 0; // Less than 18cm, set servo to 0 degrees
  } else if (dist_raw > _TARGET_HIGH) {
    servoAngle = 180; // More than 22cm, set servo to 180 degrees
  }

// Set the servo angle
myservo.writeMicroseconds(map(servoAngle, 0, 180, _DUTY_MIN, _DUTY_MAX));
